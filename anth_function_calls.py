...
class AI:
    ...
    def __init__(self, model:Optional[str] = "claude-3-haiku-20240307", max_tokens: Optional[int] = 2000):
    ...
    def call(self, message:str, tools:Optional[str]=None) ->Message:
        ...
        """
        if(tools!= None):
            response = self.__client__.messages.create(model=self.__model__, 
                                                       ...
                                                       max_tokens=self.__max_tokens__, messages=message, tools=tools)
        else:
            response = self.__client__.messages.create(model=self.__model__, max_tokens=self.__max_tokens__, messages=message)
        return response    
...
def main():
    ...
    if response.stop_reason == 'tool_use':
        ...
        if tool_name == "car_calculation":
            result = Tools.car_calculator(response.content[-1].input['part1'], response.content[-1].input['part2'])
